...
export const generatePDFOC = ({ formData, solicitudData }: GeneratePDFOCProps) => {
  const getStatus = () => {
  ...
  return (
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        {/* ===== DATOS ESPECÍFICOS DE ORDEN DE CAMBIO ===== */}
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>DATOS DE ORDEN DE CAMBIO</Text>
          {[
            { label: 'Monto Original en Pesos', value: formData.montoOriginalPesos },
            { label: 'Monto Original en Dólares', value: formData.montoOriginalDolares },
            { label: 'Monto Actualizado', value: formData.montoActualizado },
            { label: 'Monto Actualizado USD', value: formData.montoActualizadoUSD },
            { label: 'Nueva Ocupación', value: formData.nuevaOcupacion },
            { label: 'Nueva Sustancial', value: formData.nuevaSustancial },
            { label: 'Nueva Final', value: formData.nuevaFinal },
            { label: 'Tiempo (días)', value: formData.tiempoDias },
          ].map((row, index) => (
            <View
              key={index}
              style={[styles.tableRow, index % 2 === 1 ? styles.tableRowEven : {}]}
